...
def handleNominal(feature):
	uniqueValues = numpy.unique(feature)
	subFeatures = numpy.zeros(shape = (len(feature),len(uniqueValues)))
	for index,item in enumerate(feature):

		#find index of item in uniqueValues
		subFeatureIndex = numpy.where(uniqueValues == item)
		#only add make sub feature a 1 if its index is in range (handle 0 mapping)
		if(subFeatureIndex[0] <= len(uniqueValues)):
			subFeatures[index,subFeatureIndex[0]] = 1
	return subFeatures
